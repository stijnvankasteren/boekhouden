...
function applyView() {
  ...
  switch (currentView) {
    case 'income':
      titleEl.textContent = 'Verkopen & Inkomsten';
      captionEl.textContent = 'Alle inkomsten-transacties in 2025';
      txs = txs.filter((t) => t.type !== 'expense');
      break;
    case 'expense':
      titleEl.textContent = 'Inkopen & Uitgaven';
      captionEl.textContent = 'Alle uitgaven-transacties in 2025';
      txs = txs.filter((t) => t.type === 'expense');
      break;
    case 'relations':
      titleEl.textContent = 'Relaties';
      captionEl.textContent =
        'Relatiebeheer is nog niet geïmplementeerd in deze simpele versie.';
      txs = [];
      break;
    case 'wvbalans':
      titleEl.textContent = 'Winst & Verlies / Balans';
      captionEl.textContent =
        'Samenvattende rapportages volgen nog in een toekomstige versie.';
      txs = [];
      break;
    case 'btw':
      titleEl.textContent = 'Btw-aangifte';
      captionEl.textContent =
    ...
    case 'settings':
      ...
      break;
    default:
      titleEl.textContent = 'Transacties 2025';
      captionEl.textContent = 'Overzicht van alle mutaties.';
      break;
  }
